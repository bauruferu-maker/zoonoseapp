...
export default function DashboardClient({ stats, sectors }: { stats: SectorStat[]; sectors: Sector[] }) {
  ...
  const filtered = selectedSector === 'all' ? stats : stats.filter((item) => item.sector_name === selectedSector)

  const totals = filtered.reduce(
    (accumulator, item) => ({
      total: accumulator.total + item.total_visits,
      findings: accumulator.findings + item.with_findings,
      refused: accumulator.refused + item.refused,
      notFound: accumulator.notFound + item.not_found,
    }),
      ...
      <main className="mx-auto max-w-6xl p-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {sectors.map((sector) => {
              const sectorStats = stats.filter((item) => item.sector_name === sector.name)
...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <p className={`text-3xl font-bold ${colors[color]}`}>{value.toLocaleString('pt-BR')}</p>
    </div>
  )
}
