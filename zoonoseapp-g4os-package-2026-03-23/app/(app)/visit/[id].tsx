    ...
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <View style={s.statusCard}>
        ...
        <Text style={s.dateText}>
          {visit.visited_at ? new Date(visit.visited_at).toLocaleString('pt-BR') : 'Data nao registrada'}
        </Text>
      </View>

      <View style={s.section}>
        <SectionRow icon="place" label="Imovel" value={(visit as any).properties?.address ?? 'Endereco nao informado'} />
        <SectionRow icon="person" label="Agente" value={(visit as any).profiles?.name ?? 'Agente nao informado'} />
        <SectionRow icon="mail" label="Contato" value={(visit as any).profiles?.email ?? 'Email nao informado'} />
      </View>

      <View style={s.section}>
        <Text style={s.sectionTitle}>Observacoes</Text>
        <Text style={s.notesText}>{visit.notes ?? 'Nenhuma observacao registrada.'}</Text>
      </View>

      <View style={s.section}>
        <Text style={s.sectionTitle}>Geolocalizacao</Text>
        <Text style={s.geoText}>Latitude: {visit.lat ?? 'Nao capturada'}</Text>
        <Text style={s.geoText}>Longitude: {visit.lng ?? 'Nao capturada'}</Text>
      </View>
    </ScrollView>
  )
}
